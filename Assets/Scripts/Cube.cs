...
public class Cube : MonoBehaviour
{
    ...
    // TODO: Remove this. Functionality belongs in GameManager
    // The RotationSequence that is currently underway
    ...
    /// <summary>
    /// Is cube ready to rotate? Meaning cube is NOT currently rotating, and the delay has finished
    /// </summary>
    ...
    void Update()
    {
        ...
        // If the cube is currently rotating, it should continue
        if (IsRotating)
    ...
    /// <summary>
    /// Attempt to start a rotation of the cube. If the cube is not ready to rotate, no action will be taken.
    /// </summary>
    /// <param name="rotation">The rotation to attempt</param>
    public void StartRotation( CubeRotation rotation )
    {
        if( !ReadyToRotate )
        {
            Debug.Log($"Cube is not ready to rotate. This rotation will be disregarded: {rotation}.");
            return;
        }
    ...
    /// <summary>
    /// Rotate cube, interpolating the angle for Update() cycle
    /// When rotation is complete, this method will call FinishRotation()
    /// </summary>
    private void ContinueCurrentRotation()
    {
        ...
        // If the rotation angle is less than the desired angle,
        // then the rotation needs to continue
        if( currentRotationActualDegrees < currentRotation.Angle )
        {
            ...
            float angleToRotate = Time.deltaTime * DegreesPerSecond * currentRotation.Direction;

            // Iterate over the moving pieces
            ...
            foreach ( var piece in loc.GetPieces(currentRotation.FaceLike) )
            {
                ...
                piece.transform.RotateAround(Vector3.zero, currentRotation.RotationAxis, angleToRotate);
            }

            // Update the actual angle rotated
            currentRotationActualDegrees += Mathf.Abs(angleToRotate);
        }

        // If the rotation angle is >= the desired angle,
        // then the rotation has finished
        else
        {
            FinishRotation();
        }
    ...
    private void FinishRotation()
    {
        ...
        // Remove the lock on starting a new rotation
        IsRotating = false;
        
        // The next rotation can start only after the delay is completed,
        // So calculate when that will be
        NextRotationTime = DateTime.Now.AddMilliseconds(DelayMilliseconds);
    }
    ...
    /// </summary>
    /// <param name="piece">A GameObject with a .transform.rotation.eulerAngles</param>
    /// <returns></returns>
